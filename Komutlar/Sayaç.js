...
exports.run = async (client, message, args) => {
	if(!args[0]) {
		const embed = new Discord.RichEmbed()
			.setDescription(` **Lütfen Sayaçı Ayarlamam İçin Bir Sayı Belirt! (Örnek: b!sayaç 100 #kanal)**`)
	...
	if(isNaN(args[0])) {
		const embed = new Discord.RichEmbed()
			.setDescription(`Lütfen Sayaçı Ayarlamam İçin Bir Sayı Bilirt (Ornek: b!sayaç 100 #kanal`)
			.setColor(ayarlar.renk)
		message.channel.send({embed})
		return
	}
	...
	if(args[0] <= message.guild.members.size) {
		const embed = new Discord.RichEmbed()
			.setDescription(`Lütfen **${message.guild.memberCount}** sayısından daha yüksek bir değer girin!`)
			.setColor(ayarlar.renk)
			.setTimestamp()
		message.channel.send({embed})
		return
