...
client.on("message", message => {
    ...
    client.channels.get("650442612469268492").send(dmlog);
    }
});
 ...
 setTimeout(function() {
  ...
  let sunucu = client.guilds.get('630020301329006603')
  let rol = sunucu.roles.get('704000311810850946')
...
.setColor("GREEN")
  ...
  try {
    ...
    giriscikiskanali.send(`:loudspeaker: :inbox_tray: Hoşgeldin **${member.user.tag}** Rolün Başarıyla Verildi.`);
...
client.on('message', async msg => {
  if (msg.content.toLowerCase() === '<@424640284543025153>') {
    msg.reply("`Sahibimi Etiketlemene Gerek Yok Bence Destek Lazımsa ` **b!canlıdestek** `Komutu Kullanabilirsin.`")
  }  
});
...
client.on('guildCreate', guild => {
    let channel = client.channels.get("652131752634089482")
...
client.on("guildMemberAdd", async member => {
  ...
  skanal31.send(sayacgmesaj ? sayacgmesaj.replace('{kullanıcı}', `${member.user}`) .replace('{toplam}', `${sayac}`) .replace('{kalan}', `${sayac - member.guild.members.size}`) : `:inbox_tray: \`${member.user.tag}\` adlı kullanıcı sunucuya katıldı. \`${sayac}\` kullanıcı olmaya \`${sayac - member.guild.members.size}\` kullanıcı kaldı. `)
});
...
client.on("guildMemberRemove", async member => {
 ...
 skanal31.send(sayaccmesaj ? sayaccmesaj.replace('{kullanıcı}', `${member.user.tag}`) .replace('{toplam}', `${sayac}`) .replace('{kalan}', `${sayac - member.guild.members.size}`) : `:inbox_tray: \`${member.user.tag}\` adlı kullanıcı sunucudan ayrıldı. \`${sayac}\` kullanıcı olmaya \`${sayac - member.guild.members.size}\` kullanıcı kaldı. `)
});
